...
void BMP180_ReadRegisters(uint8_t address, uint8_t *data, uint8_t size) {
	...
	// I2C Send
	I2C2_Start();
	I2C2_SendAddress(BMP180_ADDRESS, I2C_TRANSMITTER);
	I2C2_SendData(address);

	// I2C Receive
	I2C2_Start();
	I2C2_SendAddress(BMP180_ADDRESS, I2C_RECEIVER);
...
uint8_t BMP180_GetStatus() {
	...
	// I2C Send
	I2C2_Start();
	if (I2C2_SendAddress(BMP180_ADDRESS, I2C_TRANSMITTER) == I2C_ERROR) return 0;
	I2C2_SendData(BMP180_REGISTER_ID);

	// I2C Receive
	I2C2_Start();
	if (I2C2_SendAddress(BMP180_ADDRESS, I2C_RECEIVER) == I2C_ERROR) return 0;
	I2C2_Stop();

	id = I2C2_ReceiveData(I2C_NACK);
	if(id != 0x58) {
		return 0;
	}
