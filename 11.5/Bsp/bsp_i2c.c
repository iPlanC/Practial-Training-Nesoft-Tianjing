...
void I2C2_Init(I2C_Mode_Type mode) {

	RCC->APB1ENR |= RCC_APB1ENR_I2C2EN;				// 1: I2C1 clock enabled
	RCC->APB2ENR |= RCC_APB2ENR_IOPBEN;				// 1: I/O port B clock enabled
	RCC->APB2ENR |= RCC_APB2ENR_AFIOEN;				// 1: Alternate Function I/O clock enabled
	...
	// I2C
	I2C2->CR1 |= I2C_CR1_SWRST;						// 1: I2C Peripheral under reset state
	I2C2->CR1 = 0;									// Reset

	I2C2->CR2 |= (PCLK1_FREQUENCY / 1000000);		// FREQ[5:0]: Peripheral clock frequency

	if (mode == I2C_STANDARD) {
		I2C2->CCR = (PCLK1_FREQUENCY / (100000 * 2));					// CCR[11:0]: Clock control register in Fm/Sm mode (Master mode) 100kHz
		I2C2->TRISE = ((PCLK1_FREQUENCY / 1000000) + 1);				// TRISE[5:0]: Maximum rise time in Fm/Sm mode (Master mode)
	} else if (mode == I2C_FAST) {
		I2C2->CCR = (PCLK1_FREQUENCY / (400000 * 3));					// CCR[11:0]: Clock control register in Fm/Sm mode (Master mode) 400kHz
		I2C2->CCR |= I2C_CCR_FS;										// 1: Fm mode I2C (Fast mode - 400kHz)
		I2C2->TRISE = (((PCLK1_FREQUENCY / 1000000 * 300) / 1000) + 1);	// TRISE[5:0]: Maximum rise time in Fm/Sm mode (Master mode)
	}
